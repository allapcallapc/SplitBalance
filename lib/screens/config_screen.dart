...
class _ConfigScreenState extends State<ConfigScreen> {
  ...
  List<drive.File> _selectedFolderPath =
      <drive.File>[]; // Full path from root to selected folder
  ...
  bool _hasLoadedCategoriesInBuilder =
      false; // Flag to prevent infinite loadCategories calls
  String?
      _lastLoadedCategoriesFolderId; // Track which folder we loaded categories for
  // Navigation stack for folder browsing (breadcrumb)
  List<drive.File> _folderPath =
      <drive.File>[]; // Stack of folders we've navigated into
  String _appVersion = ''; // App version string
  bool _hasAttemptedLoadFolders =
      false; // Flag to prevent infinite retry loop when folder loading fails
  ...
  void didChangeDependencies() {
    ...
    if (!_controllersInitialized && !configProvider.isLoading) {
      ...
      if (configProvider.isSignedIn &&
        ...
        _hasAttemptedLoadFolders =
            true; // Set flag immediately to prevent repeated attempts
        WidgetsBinding.instance.addPostFrameCallback((_) {
          if (mounted) {
  ...
  void _onConfigChanged() {
    if (!mounted || _configProvider == null || _configProvider!.isLoading)
      return;
    ...
    if (!provider.isSignedIn &&
        (_hasShownFolderPrompt || _hasAttemptedLoadFolders)) {
        ...
        WidgetsBinding.instance.addPostFrameCallback((_) {
          if (mounted) {
            _loadFolders(provider, null).then((_) {
              ...
              if (mounted &&
                  provider.isSignedIn &&
                  provider.driveService.folderId == null &&
                  _hasFolders &&
                  !_hasShownFolderPrompt) {
  ...
  Future<void> _promptFolderSelection(ConfigProvider configProvider) async {
      ...
      builder: (context) => AlertDialog(
        title: Row(
          children: [
            ...
            Expanded(
                child: Text(AppLocalizations.of(context)!.selectStorageFolder)),
          ],
        ),
        content: SizedBox(
                              ...
                              return ListTile(
                                ...
                                onTap: () async {
                                  ...
                                  if (folder.id != null) {
                                    ...
                                    _hasShownFolderPrompt =
                                        false; // Reset flag after selection
                                    ...
                                    // This will check if current person names match the new folder and only prompt if needed
                                    if (mounted) {
                                      _checkFolderDataAndRestoreState(
                                          configProvider);
                                    }
                                  }
                                  if (context.mounted) {
                                    Navigator.pop(context,
                                        'selected'); // Return 'selected' to indicate folder was chosen
        ...
        actions: [
          ...
          ElevatedButton.icon(
            onPressed: () async {
              Navigator.pop(context,
                  'refresh'); // Refresh - return 'refresh' to indicate refresh was clicked
              _hasAttemptedLoadFolders =
                  false; // Reset flag to allow manual refresh
              await _loadFolders(configProvider, null);
    ...
    if (result == 'selected') {
      ...
      _hasShownFolderPrompt =
          false; // Reset to allow re-prompting after refresh
    } else {
  ...
  Future<void> _checkFolderDataAndRestoreState(
      ConfigProvider configProvider) async {
    if (_isCheckingFolderData ||
        !mounted ||
        !configProvider.isSignedIn ||
        configProvider.driveService.folderId == null) {
      return;
    ...
    try {
      ...
      try {
        final personNamesCsv =
            await configProvider.driveService.downloadPersonNames();
        if (personNamesCsv.isNotEmpty) {
          ...
          if (drivePerson1.isNotEmpty || drivePerson2.isNotEmpty) {
            ...
            print(
                'Person names loaded from Drive folder and updated in UI: Person1=$drivePerson1, Person2=$drivePerson2');
          }
      ...
      final currentPerson1 = hasDrivePersonNames
          ? drivePerson1
          : configProvider.config.person1Name.trim();
      final currentPerson2 = hasDrivePersonNames
          ? drivePerson2
          : configProvider.config.person2Name.trim();
      final hasCurrentNames =
          currentPerson1.isNotEmpty && currentPerson2.isNotEmpty;
      ...
      final shouldSkipPrompt =
          hasCurrentNames && (namesMatch || !hasPersonNamesCsv);
      ...
      if (shouldSkipPrompt) {
        ...
        if (mounted) {
          await _checkAndForceCategorySelection(
              configProvider, categoriesProvider);
        }
      } else {
        ...
        if (!_hasShownPersonNamePrompt && mounted) {
          ...
          if (mounted) {
            _promptPersonNameSelection(
                configProvider, personNamesList, hasPersonNamesCsv);
          }
        }
      }
    } catch (e) {
  ...
  Future<void> _checkAndForceCategorySelection(ConfigProvider configProvider,
  ...
  Future<void> _promptPersonNameSelection(ConfigProvider configProvider,
      List<String> suggestedNames, bool hasPersonNamesCsv) async {
      ...
      builder: (context) => AlertDialog(
        title: Row(
          children: [
            ...
            const SizedBox(width: 12),
            Expanded(
                child: Text(AppLocalizations.of(context)!.enterPersonNames)),
          ],
        ),
        content: SizedBox(
          width: double.maxFinite,
            ...
            onPressed: () async {
              ...
              if (mounted) {
                ...
                if (mounted && configProvider.isSignedIn) {
                  ...
                  _hasShownFolderPrompt =
                      false; // Reset to allow showing dialog
                  if (mounted) {
                    _promptFolderSelection(configProvider);
                  }
                }
    ...
    if (mounted && result == true) {
      _hasShownPersonNamePrompt =
          false; // Reset to allow re-prompting if needed
  ...
  Future<void> _promptCategorySelection(ConfigProvider configProvider,
      CategoriesProvider categoriesProvider) async {
    if (!mounted ||
        !configProvider.isSignedIn ||
        configProvider.driveService.folderId == null) {
        ...
        return AlertDialog(
          ...
          actions: [
            TextButton(
              onPressed: () {
                Navigator.pop(
                    dialogContext, 'cancel'); // Cancel - allow going back
              },
              child: Text(l10n.cancel),
            ),
            TextButton(
            ...
            ElevatedButton(
              onPressed: () {
                Navigator.pop(dialogContext,
                    'navigate'); // Return 'navigate' to indicate we should navigate
              },
              style: ElevatedButton.styleFrom(
  ...
  Future<void> _loadSelectedFolderPath(ConfigProvider configProvider) async {
    ...
    try {
      ...
      final selectedFolder =
          await configProvider.driveService.getFolder(currentFolderId!);
      ...
      setState(() {
        ...
        _selectedFolderName =
            path.isNotEmpty ? (path.last.name ?? folderName) : folderName;
        _loadingSelectedFolderPath = false;
      });
    } catch (e) {
      ...
      try {
        if (_selectedFolderId != null) {
          final folder =
              await configProvider.driveService.getFolder(_selectedFolderId!);
          if (folder != null && folder.name != null) {
            setState(() {
              _selectedFolderName = folder.name;
              _selectedFolderPath =
                  <drive.File>[]; // Clear path if we couldn't build it
              _loadingSelectedFolderPath = false;
            });
            return;
          }
  ...
  Future<void> _loadFolders(ConfigProvider configProvider,
    ...
    try {
      final folders =
          await configProvider.driveService.listFolders(parentFolderId);
      setState(() {
      ...
      if (mounted) {
        ...
        if (errorString.contains('Bearer null') ||
          ...
          errorMessage =
              'Authentication failed: The "Continue as" sign-in method is not fully supported. Please sign out and sign in again using the full Google sign-in popup.';
        } else {
  ...
  Future<void> _navigateIntoFolder(
      ConfigProvider configProvider, drive.File folder) async {
  ...
  Future<void> _createFolder(ConfigProvider configProvider) async {
    ...
    try {
      ...
      await configProvider.driveService
  ...
  Future<void> _saveConfig(ConfigProvider configProvider) async {
    try {
      ...
      if (mounted) {
        ...
        if (saved &&
            person1Name.isNotEmpty &&
            person2Name.isNotEmpty &&
            configProvider.driveService.folderId != null) {
          final categoriesProvider = context.read<CategoriesProvider>();
          await _checkAndForceCategorySelection(
              configProvider, categoriesProvider);
      ...
      body: Consumer2<ConfigProvider, CategoriesProvider>(
        builder: (context, configProvider, categoriesProvider, child) {
          ...
          if (configProvider.isSignedIn &&
            ...
            _hasLoadedCategoriesInBuilder =
                true; // Set flag immediately to prevent duplicate scheduling
                    ...
                    child: Padding(
                      ...
                      child: Column(
                        ...
                        children: [
                          ...
                          Text(
                            AppLocalizations.of(context)!
                                .selectGoogleDriveFolderMessage,
                            style: TextStyle(
                              fontSize: 14,
                      ...
                      children: [
                        Row(
                          children: [
                            Icon(
                              Icons.cloud_upload,
                              color: configProvider.isSignedIn
                                  ? Colors.green
                                  : Colors.blue,
                              size: 24,
                            ),
                        ...
                        if (configProvider.isSignedIn) ...[
                          ...
                          ElevatedButton.icon(
                            onPressed: configProvider.isLoading
                                ...
                                : () async {
                                    final success =
                                        await configProvider.signIn();
                                    if (success && context.mounted) {
                                      _hasAttemptedLoadFolders =
                                          false; // Reset flag after successful sign-in
                        ...
                        if (configProvider.restoreError != null &&
                            !configProvider.isSignedIn) ...[
                          const SizedBox(height: 8),
                          Card(
                            color: Colors.orange[50],
                            child: Padding(
                              ...
                              child: Column(
                                ...
                                children: [
                                    ...
                                    children: [
                                      ...
                                      IconButton(
                                        icon: Icon(Icons.close,
                                            ...
                                            color: Colors.orange[700]),
                                        onPressed: () =>
                                            configProvider.clearRestoreError(),
                                  ...
                                  if (configProvider.restoreErrorDetails !=
                                      null) ...[
                                    const SizedBox(height: 8),
                                    ExpansionTile(
                                      title: const Text(
                              ...
                              child: Padding(
                                ...
                                child: Column(
                                  ...
                                  children: [
                                    ...
                                    if (_loadingSelectedFolderPath) ...[
                                      ...
                                      Builder(
                                        builder: (context) {
                                          ...
                                          if (_selectedFolderId != null &&
                                              _selectedFolderName == null &&
                                              !_loadingSelectedFolderPath) {
                                            WidgetsBinding.instance
                                                .addPostFrameCallback((_) {
                                              if (mounted) {
                                                _loadSelectedFolderPath(
                                                    configProvider);
                                              }
                                            });
                                          }
                                    ...
                                    const SizedBox(height: 8),
                                    ElevatedButton.icon(
                                      onPressed: _loadingSelectedFolderPath ||
                                              !_hasSelectedFolderPath
                                          ? null
                                          : () async {
                              ...
                              child: ListView.builder(
                                ...
                                itemBuilder: (context, index) {
                                  ...
                                  return Card(
                                    ...
                                    child: ListTile(
                                      title:
                                          Text(folder.name ?? itemL10n.unnamed),
                                      leading: const Icon(Icons.folder,
                                          color: Colors.blue),
                                      trailing: Row(
                                        mainAxisSize: MainAxisSize.min,
                                        children: [
                                          ...
                                          IconButton(
                                            ...
                                            onPressed: folder.id != null
                                                ? () async {
                                                    await _navigateIntoFolder(
                                                        configProvider, folder);
                                                  }
                                                : null,
                                            tooltip:
                                                itemL10n.navigateIntoFolder,
                                          ),
                                          IconButton(
                                                ...
                                                ? () async {
                                                    ...
                                                    if (context.mounted) {
                                                      ...
                                                      ScaffoldMessenger.of(
                                                          ...
                                                          .showSnackBar(
                                                        SnackBar(
                                                          content: Text(snackL10n
                                                              .folderSelectedAndSaved(
                                                                  folder.name ??
                                                                      itemL10n
                                                                          .unnamed)),
                                                          backgroundColor:
                      ...
                      child: Column(
                        ...
                        children: [
                              ...
                              child: Row(
                                children: [
                                  ...
                                  Expanded(
                                    child: Text(
                                      AppLocalizations.of(context)!
                                          .selectFolderFirstToEnterPersonNames,
                                      style: TextStyle(
                                        fontSize: 13,
                                        color: Colors.orange[900],
                                      ),
                          ...
                          TextField(
                            ...
                            decoration: InputDecoration(
                              labelText:
                                  AppLocalizations.of(context)!.person1Name,
                              border: const OutlineInputBorder(),
                              hintText:
                                  configProvider.driveService.folderId == null
                                      ? AppLocalizations.of(context)!
                          ...
                          TextField(
                            ...
                            decoration: InputDecoration(
                              labelText:
                                  AppLocalizations.of(context)!.person2Name,
                              border: const OutlineInputBorder(),
                              hintText:
                                  configProvider.driveService.folderId == null
                                      ? AppLocalizations.of(context)!
