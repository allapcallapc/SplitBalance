...
class ConfigProvider with ChangeNotifier {
  ...
  Future<void> _loadConfig() async {
    ...
    try {
      ...
      // Try to restore Google Sign-In session silently
      // IMPORTANT: Google sessions persist in browser storage/cookies, so we only need to
      // restore our Dart objects (_currentUser, _driveApi) which are lost on page reload.
      // However, signInSilently() makes a network request which can trigger rate limiting.
      // Therefore, we should:
      // 1. Only attempt restore if we have a stored "was_signed_in" flag
      // 2. Respect rate limiting by not attempting too frequently
      // 3. Trust that Google's session persists in browser storage
      
      final wasSignedIn = prefs.getBool('was_signed_in') ?? false;
      final lastRestoreTime = prefs.getInt('last_restore_attempt_time');
      
      print('Previous sign-in status flag: $wasSignedIn');
      
      // Check if we should attempt restore (avoid rate limiting)
      bool shouldAttemptRestore = wasSignedIn;
      if (lastRestoreTime != null) {
        final lastRestore = DateTime.fromMillisecondsSinceEpoch(lastRestoreTime);
        final timeSinceLastAttempt = DateTime.now().difference(lastRestore);
        if (timeSinceLastAttempt.inMinutes < 10) {
          print('⏱️  Last restore attempt was ${timeSinceLastAttempt.inMinutes} minutes ago');
          print('⏱️  Skipping restore to avoid rate limiting (Google limits to once per 10 minutes)');
          print('💡 Google session persists in browser storage - user can click "Sign In" if needed');
          shouldAttemptRestore = false;
        }
      }
      
      String? restoreError;
      String? restoreErrorDetails;
      
      if (shouldAttemptRestore) {
        // Try to restore the session automatically
        // This restores our Dart objects from Google's persistent browser session
        try {
          print('=== Attempting to restore Google Sign-In session ===');
          print('Note: Google session persists in browser storage - this just restores our Dart objects');
          
          // Record attempt time to avoid rate limiting
          await prefs.setInt('last_restore_attempt_time', DateTime.now().millisecondsSinceEpoch);
          
          final restored = await _driveService.signInSilently();
          if (restored) {
            print('✅ Google Sign-In session restored successfully');
            print('✅ Dart objects restored - session will persist in browser storage');
          ...
          // signInSilently() handles FedCM errors internally, so any exception here
          // means the session truly couldn't be restored
          print('❌ Exception during session restoration: $e');
          print('Stack trace: $stackTrace');
          restoreError = e.toString();
          restoreErrorDetails = 'Exception: ${e.runtimeType}';
          // Don't clear was_signed_in flag - session may still be valid in browser
        }
      } else if (wasSignedIn) {
        // We have was_signed_in flag but skipped restore due to rate limiting
        // Google session should still persist in browser storage
        print('ℹ️  Skipped restore attempt (rate limiting protection)');
        print('ℹ️  Google session should persist in browser storage');
        print('💡 User can click "Sign In" button to manually verify/restore session');
        // Don't show error - session may still be valid
      } else {
        // No previous session flag - user needs to sign in
        print('ℹ️  No previous session detected - user needs to sign in');
      }
      
      // Store restore error for UI display if session restoration failed
      // Only store errors if we actually attempted restore (not if we skipped due to rate limiting)
      if (restoreError != null && shouldAttemptRestore) {
        // Store error even if it's FedCM-related, but mark it appropriately
        ...
        // No error or we skipped restore (rate limiting) - don't show error
        _restoreError = null;
        _restoreErrorDetails = null;
        if (!shouldAttemptRestore && wasSignedIn) {
          print('ℹ️  Skipped restore due to rate limiting - not showing error (session may still be valid)');
        }
      }
